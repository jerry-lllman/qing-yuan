...
/** 获取好友列表 */
export const useFriends = () => useContactStore((state) => state.friends);

/** 获取收到的好友请求 */
export const useReceivedRequests = () => useContactStore((state) => state.receivedRequests);

/** 获取发出的好友请求 */
export const useSentRequests = () => useContactStore((state) => state.sentRequests);

/** 获取待处理的好友请求数量 */
export const usePendingRequestCount = () =>
  useContactStore((state) => state.receivedRequests.filter((r) => r.status === 'pending').length);

/** 获取黑名单 */
export const useBlockedUsers = () => useContactStore((state) => state.blockedUsers);

/** 获取用户在线状态 */
export const useUserOnlineStatus = (userId: string) =>
  useContactStore((state) => state.onlineStatuses.get(userId));

/** 检查是否是好友 */
export const useIsFriend = (userId: string) =>
  useContactStore((state) => state.friends.some((f) => f.friendId === userId));

/** 检查是否已拉黑 */
export const useIsBlocked = (userId: string) =>
  useContactStore((state) => state.blockedUsers.some((u) => u.userId === userId));

/** 获取好友列表加载状态 */
export const useIsLoadingFriends = () => useContactStore((state) => state.isLoadingFriends);

/** 获取请求列表加载状态 */
export const useIsLoadingRequests = () => useContactStore((state) => state.isLoadingRequests);
...
export const useSortedFriends = () =>
  useContactStore((state) =>
    [...state.friends].sort((a, b) => {
      // 优先显示备注名，否则显示昵称
      const nameA = a.remark || a.friend.nickname;
      const nameB = b.remark || b.friend.nickname;
      return nameA.localeCompare(nameB, 'zh-CN');
    })
  );

/** 按在线状态排序的好友列表（在线优先） */
export const useFriendsSortedByStatus = () =>
  useContactStore((state) => {
    ...
    return [...state.friends].sort((a, b) => {
      const statusA = state.onlineStatuses.get(a.friendId)?.status || 'offline';
      const statusB = state.onlineStatuses.get(b.friendId)?.status || 'offline';
