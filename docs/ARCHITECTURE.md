# Cyan - 多端即时通讯应用架构文档

> 一款支持 iOS、Android、PC 的即时通讯软件，采用 TypeScript 全栈开发

---

## 目录

1. [技术栈概览](#1-技术栈概览)
2. [项目结构](#2-项目结构)
3. [架构设计](#3-架构设计)
4. [包依赖关系](#4-包依赖关系)
5. [开发计划](#5-开发计划)
6. [模块详细说明](#6-模块详细说明)
7. [消息同步与离线策略](#7-消息同步与离线策略)
8. [API 版本管理](#8-api-版本管理)
9. [监控与可观测性](#9-监控与可观测性)
10. [开发规范](#10-开发规范)
11. [部署架构](#11-部署架构)

---

## 1. 技术栈概览

| 层级         | 技术选型                                 |
| ------------ | ---------------------------------------- |
| **桌面端**   | Electron + Vite + React                  |
| **移动端**   | React Native (iOS & Android)             |
| **后端**     | Nest.js + Prisma + PostgreSQL            |
| **实时通信** | Socket.io (WebSocket)                    |
| **状态管理** | Zustand + TanStack Query                 |
| **构建工具** | pnpm + Turborepo                         |
| **类型验证** | Zod                                      |
| **样式方案** | Tailwind CSS (Web) / StyleSheet (Native) |
| **组件库**   | shadcn/ui (Web) / Gluestack UI (Native)  |

---

## 2. 项目结构

```
cyan/
├── apps/                               # 🚀 应用层（可独立部署的应用）
│   │
│   ├── desktop/                        # 💻 Electron 桌面应用
│   │   ├── src/
│   │   │   ├── main/                   # Electron 主进程
│   │   │   │   ├── index.ts            # 主进程入口
│   │   │   │   ├── ipc/                # IPC 通信处理
│   │   │   │   ├── tray/               # 系统托盘
│   │   │   │   └── updater/            # 自动更新
│   │   │   ├── preload/                # 预加载脚本（安全桥接）
│   │   │   │   └── index.ts
│   │   │   └── renderer/               # Electron 渲染进程
│   │   │       ├── App.tsx             # 引入 ui-web + client-state
│   │   │       ├── pages/              # 页面组件
│   │   │       └── router/             # 路由配置
│   │   ├── resources/                  # 应用资源（图标等）
│   │   ├── electron.vite.config.ts
│   │   └── package.json
│   │
│   ├── mobile/                         # 📱 React Native 移动应用
│   │   ├── src/
│   │   │   ├── App.tsx                 # 引入 ui-native + client-state
│   │   │   ├── screens/                # 屏幕组件
│   │   │   ├── navigation/             # React Navigation 配置
│   │   │   ├── native-modules/         # 原生模块桥接
│   │   │   └── services/               # 移动端特有服务
│   │   │       ├── push/               # 推送通知处理
│   │   │       ├── background/         # 后台任务
│   │   │       └── biometrics/         # 生物识别认证
│   │   ├── ios/                        # iOS 原生代码
│   │   ├── android/                    # Android 原生代码
│   │   ├── metro.config.js             # Metro 打包配置
│   │   └── package.json
│   │
│   └── server/                         # 🔧 Nest.js 后端服务
│       ├── src/
│       │   ├── main.ts                 # 服务入口
│       │   ├── app.module.ts
│       │   ├── modules/
│       │   │   ├── auth/               # 认证模块 (JWT, OAuth)
│       │   │   ├── user/               # 用户模块
│       │   │   ├── friend/             # 好友关系
│       │   │   ├── chat/               # 私聊
│       │   │   ├── group/              # 群聊
│       │   │   ├── message/            # 消息存储/检索
│       │   │   ├── file/               # 文件上传/下载
│       │   │   └── notification/       # 推送通知 (FCM/APNs)
│       │   ├── gateway/                # WebSocket 网关
│       │   │   ├── chat.gateway.ts     # 聊天 WS 网关
│       │   │   └── notification.gateway.ts
│       │   ├── common/
│       │   │   ├── guards/             # 守卫
│       │   │   ├── interceptors/       # 拦截器
│       │   │   ├── filters/            # 异常过滤器
│       │   │   └── decorators/         # 自定义装饰器
│       │   └── config/                 # 配置管理
│       ├── prisma/                     # Prisma ORM
│       │   └── schema.prisma
│       ├── nest-cli.json
│       └── package.json
│
├── packages/                           # 📦 库层（可复用的代码包）
│   │
│   ├── shared/                         # 🧠 纯 TS 逻辑（零框架依赖）
│   │   ├── src/
│   │   │   ├── types/                  # 类型定义
│   │   │   │   ├── user.ts
│   │   │   │   ├── message.ts
│   │   │   │   ├── chat.ts
│   │   │   │   └── api.ts
│   │   │   ├── constants/              # 常量定义
│   │   │   │   ├── errors.ts           # 错误码
│   │   │   │   └── limits.ts           # 业务限制常量
│   │   │   ├── utils/                  # 工具函数
│   │   │   │   ├── date.ts
│   │   │   │   ├── format.ts
│   │   │   │   └── validator.ts
│   │   │   └── index.ts
│   │   ├── package.json                # 零依赖
│   │   └── tsconfig.json
│   │
│   ├── protocol/                       # 📡 通信协议（前后端共享）
│   │   ├── src/
│   │   │   ├── events/                 # WebSocket 事件定义
│   │   │   │   ├── message.events.ts   # 消息相关事件
│   │   │   │   ├── chat.events.ts      # 会话相关事件
│   │   │   │   ├── user.events.ts      # 用户状态事件
│   │   │   │   └── index.ts
│   │   │   ├── payloads/               # 事件载荷类型
│   │   │   │   ├── message.payload.ts
│   │   │   │   └── chat.payload.ts
│   │   │   ├── schemas/                # Zod 验证 schema
│   │   │   │   ├── auth.schema.ts
│   │   │   │   ├── message.schema.ts
│   │   │   │   └── user.schema.ts
│   │   │   └── index.ts
│   │   ├── package.json                # 依赖: zod
│   │   └── tsconfig.json
│   │
│   ├── client-core/                    # 🔌 客户端核心（网络 + 存储）
│   │   ├── src/
│   │   │   ├── api/                    # HTTP API 封装
│   │   │   │   ├── client.ts           # Axios 实例
│   │   │   │   ├── auth.api.ts
│   │   │   │   ├── user.api.ts
│   │   │   │   └── chat.api.ts
│   │   │   ├── socket/                 # WebSocket 封装
│   │   │   │   ├── socket-client.ts    # Socket.io 客户端
│   │   │   │   └── event-handlers.ts
│   │   │   ├── storage/                # 本地存储抽象
│   │   │   │   ├── adapter.ts          # 存储适配器接口
│   │   │   │   ├── web-adapter.ts      # IndexedDB 实现
│   │   │   │   └── native-adapter.ts   # MMKV/AsyncStorage 实现
│   │   │   └── sync/                   # 数据同步
│   │   │       ├── message-sync.ts     # 消息同步策略
│   │   │       └── offline-queue.ts    # 离线消息队列
│   │   ├── package.json                # 依赖: shared, protocol, axios, socket.io-client
│   │   └── tsconfig.json
│   │
│   ├── client-state/                   # 🏪 前端状态管理
│   │   ├── src/
│   │   │   ├── stores/                 # Zustand Stores
│   │   │   │   ├── auth.store.ts
│   │   │   │   ├── user.store.ts
│   │   │   │   ├── chat.store.ts
│   │   │   │   ├── message.store.ts
│   │   │   │   └── ui.store.ts
│   │   │   ├── hooks/                  # 业务 Hooks
│   │   │   │   ├── useAuth.ts
│   │   │   │   ├── useChat.ts
│   │   │   │   ├── useMessages.ts
│   │   │   │   └── useSocket.ts
│   │   │   ├── queries/                # TanStack Query
│   │   │   │   ├── user.queries.ts
│   │   │   │   └── chat.queries.ts
│   │   │   └── index.ts
│   │   ├── package.json                # 依赖: shared, client-core, zustand, @tanstack/react-query
│   │   └── tsconfig.json
│   │
│   ├── encryption/                     # 🔐 端到端加密
│   │   ├── src/
│   │   │   ├── signal/                 # Signal Protocol 实现
│   │   │   │   ├── keys.ts             # 密钥生成与管理
│   │   │   │   ├── session.ts          # 会话管理
│   │   │   │   └── ratchet.ts          # Double Ratchet 算法
│   │   │   ├── crypto/                 # 加密原语
│   │   │   │   ├── aes.ts              # AES 加密
│   │   │   │   ├── curve.ts            # Curve25519
│   │   │   │   └── hash.ts             # 哈希函数
│   │   │   └── index.ts
│   │   ├── package.json                # 依赖: shared, protocol, @aspect-build/libsignal-client
│   │   └── tsconfig.json
│   │
│   ├── ui-web/                         # 🖥️ Web/Electron UI 组件库
│   │   ├── src/
│   │   │   ├── components/
│   │   │   │   ├── base/               # 基础组件 (shadcn/ui)
│   │   │   │   │   ├── Button/
│   │   │   │   │   ├── Input/
│   │   │   │   │   ├── Avatar/
│   │   │   │   │   └── Modal/
│   │   │   │   ├── chat/               # 聊天组件（自定义实现）
│   │   │   │   │   ├── ChatBubble/
│   │   │   │   │   ├── ChatInput/
│   │   │   │   │   ├── ChatList/
│   │   │   │   │   └── MessageList/
│   │   │   │   └── layout/             # 布局组件
│   │   │   │       ├── Sidebar/
│   │   │   │       └── Header/
│   │   │   ├── styles/
│   │   │   │   ├── tokens.css          # 设计令牌
│   │   │   │   └── global.css
│   │   │   └── index.ts
│   │   ├── components.json             # shadcn/ui 配置
│   │   ├── package.json                # 依赖: shared, react, tailwindcss, @radix-ui/*
│   │   └── tsconfig.json
│   │
│   ├── ui-native/                      # 📱 React Native UI 组件库
│   │   ├── src/
│   │   │   ├── components/
│   │   │   │   ├── ui/                 # Gluestack UI v3 组件 (copy-paste)
│   │   │   │   ├── chat/               # 聊天组件（自定义实现）
│   │   │   │   └── layout/
│   │   │   ├── theme/
│   │   │   │   └── tokens.ts
│   │   │   └── index.ts
│   │   ├── package.json                # 依赖: shared, react-native, nativewind
│   │   └── tsconfig.json
│   │
│   ├── testing/                        # 🧪 测试工具包
│   │   ├── src/
│   │   │   ├── factories/              # 数据工厂
│   │   │   │   ├── user.factory.ts
│   │   │   │   ├── message.factory.ts
│   │   │   │   └── chat.factory.ts
│   │   │   ├── mocks/                  # Mock 实现
│   │   │   │   ├── socket.mock.ts
│   │   │   │   ├── storage.mock.ts
│   │   │   │   └── api.mock.ts
│   │   │   └── helpers/                # 测试辅助函数
│   │   │       └── test-utils.ts
│   │   ├── package.json                # 依赖: vitest, @testing-library/*
│   │   └── tsconfig.json
│   │
│   ├── typescript-config/              # 📝 共享 TypeScript 配置
│   │   ├── base.json
│   │   ├── react.json
│   │   ├── react-native.json
│   │   ├── node.json
│   │   └── package.json
│   │
│   ├── eslint-config/                  # 📏 共享 ESLint 配置
│   │   ├── base.js
│   │   ├── react.js
│   │   ├── node.js
│   │   └── package.json
│   │
│   └── prettier-config/                # 🎨 共享 Prettier 配置
│       ├── index.js
│       └── package.json
│
├── .github/                            # GitHub 配置
│   └── workflows/
│       ├── ci.yml                      # CI 流程
│       ├── release-desktop.yml         # 桌面端发布
│       ├── release-mobile.yml          # 移动端发布
│       └── release-server.yml          # 服务端发布
│
├── turbo.json                          # Turborepo 配置
├── pnpm-workspace.yaml                 # pnpm workspace 配置
├── package.json                        # 根 package.json
├── tsconfig.json                       # 根 TS 配置
├── .env.example                        # 环境变量模板
└── README.md
```

---

## 3. 架构设计

### 3.1 整体架构图

```
┌─────────────────────────────────────────────────────────────────────────┐
│                              客户端层                                    │
├─────────────────┬─────────────────┬─────────────────────────────────────┤
│   Desktop App   │   Mobile App    │                                     │
│   (Electron)    │ (React Native)  │                                     │
│        │        │        │        │                                     │
│   ui-web ◄──────┼── ui-native     │                                     │
│  (shadcn/ui)    │  (Gluestack)    │                                     │
│        │        │        │        │                                     │
│        └────────┴────────┘        │                                     │
│                 │                 │                                     │
│          client-state             │    Zustand + TanStack Query         │
│                 │                 │                                     │
│          client-core              │    API + Socket + Storage           │
│                 │                 │                                     │
│           encryption              │    E2E 加密                          │
│                 │                 │                                     │
├─────────────────┴─────────────────┴─────────────────────────────────────┤
│                           protocol (共享协议)                             │
├─────────────────────────────────────────────────────────────────────────┤
│                           shared (共享类型/工具)                          │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                         HTTP REST + WebSocket
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                              服务端层                                    │
├─────────────────────────────────────────────────────────────────────────┤
│                          Nest.js Server                                 │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │                        Gateway Layer                              │  │
│  │   chat.gateway.ts    notification.gateway.ts                      │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │                        Module Layer                               │  │
│  │   auth │ user │ friend │ chat │ group │ message │ file │ notif   │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │                        Common Layer                               │  │
│  │   guards │ interceptors │ filters │ decorators │ config          │  │
│  └──────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                              Prisma ORM
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                              数据层                                      │
├──────────────────┬──────────────────┬───────────────────────────────────┤
│    PostgreSQL    │      Redis       │         Object Storage            │
│    (主数据库)     │    (缓存/会话)    │      (文件/图片/视频)              │
└──────────────────┴──────────────────┴───────────────────────────────────┘
```

### 3.2 消息流转架构

```
发送消息流程:
┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
│  Client  │────▶│ Encrypt  │────▶│  Socket  │────▶│  Server  │
│   Input  │     │  (E2E)   │     │   Send   │     │ Gateway  │
└──────────┘     └──────────┘     └──────────┘     └──────────┘
                                                         │
                                                         ▼
┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
│  Client  │◀────│ Decrypt  │◀────│  Socket  │◀────│ Broadcast│
│   Show   │     │  (E2E)   │     │ Receive  │     │ to Users │
└──────────┘     └──────────┘     └──────────┘     └──────────┘

离线消息流程:
┌──────────┐     ┌──────────┐     ┌──────────┐
│  Client  │────▶│  Offline │────▶│  Server  │
│  Online  │     │   Sync   │     │   Pull   │
└──────────┘     └──────────┘     └──────────┘
                      │
                      ▼
              ┌──────────────┐
              │ Local Storage│
              │    Update    │
              └──────────────┘
```

### 3.3 客户端状态架构

```
┌─────────────────────────────────────────────────────────────────┐
│                         UI Layer                                 │
│   ┌─────────────┐  ┌─────────────┐  ┌─────────────┐            │
│   │  ChatList   │  │ MessageList │  │   Sidebar   │            │
│   └──────┬──────┘  └──────┬──────┘  └──────┬──────┘            │
└──────────┼────────────────┼────────────────┼────────────────────┘
           │                │                │
           ▼                ▼                ▼
┌─────────────────────────────────────────────────────────────────┐
│                      Hooks Layer                                 │
│   ┌─────────────┐  ┌─────────────┐  ┌─────────────┐            │
│   │   useChat   │  │ useMessages │  │   useAuth   │            │
│   └──────┬──────┘  └──────┬──────┘  └──────┬──────┘            │
└──────────┼────────────────┼────────────────┼────────────────────┘
           │                │                │
           ▼                ▼                ▼
┌─────────────────────────────────────────────────────────────────┐
│                      State Layer                                 │
│   ┌──────────────────────────────────────────────────────────┐  │
│   │                    Zustand Stores                        │  │
│   │  auth.store │ chat.store │ message.store │ ui.store      │  │
│   └──────────────────────────────────────────────────────────┘  │
│   ┌──────────────────────────────────────────────────────────┐  │
│   │                   TanStack Query                         │  │
│   │  user.queries │ chat.queries │ message.queries           │  │
│   └──────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
           │                │                │
           ▼                ▼                ▼
┌─────────────────────────────────────────────────────────────────┐
│                     Core Layer                                   │
│   ┌─────────────┐  ┌─────────────┐  ┌─────────────┐            │
│   │  API Client │  │   Socket    │  │   Storage   │            │
│   └─────────────┘  └─────────────┘  └─────────────┘            │
└─────────────────────────────────────────────────────────────────┘
```

---

## 4. 包依赖关系

```
                         ┌─────────────────┐
                         │     shared      │  (零依赖)
                         └────────┬────────┘
                                  │
              ┌───────────────────┼───────────────────┐
              │                   │                   │
              ▼                   ▼                   ▼
      ┌───────────┐        ┌───────────┐       ┌───────────┐
      │ protocol  │        │  testing  │       │           │
      └─────┬─────┘        └───────────┘       │           │
              │                                 │           │
              ├─────────────────────────────────┤           │
              │                                 │           │
              ▼                                 ▼           │
      ┌─────────────┐                   ┌─────────────┐    │
      │ encryption  │ ◀─────────────────│             │    │
      │ (Signal)    │                   │             │    │
      └──────┬──────┘                   │             │    │
              │                          │             │    │
              └──────────────┬───────────┘             │    │
                             │                          │    │
                             ▼                          │    │
                      ┌─────────────┐                  │    │
                      │ client-core │ ◀────────────────┘    │
                      └──────┬──────┘                       │
                             │                               │
                             ▼                               │
                      ┌─────────────┐                       │
                      │client-state │                       │
                      └──────┬──────┘                       │
                             │                               │
               ┌─────────────┴─────────────┐                │
               │                           │                │
               ▼                           ▼                │
        ┌─────────────┐             ┌─────────────┐        │
        │   ui-web    │ ◀───────────│  ui-native  │ ◀──────┘
        │ (shadcn/ui) │   shared    │ (Gluestack) │   shared
        └──────┬──────┘             └──────┬──────┘
               │                           │
               ▼                           ▼
        ┌─────────────┐             ┌─────────────┐
        │   desktop   │             │   mobile    │
        └─────────────┘             └─────────────┘

服务端分支:
┌─────────────────┐
│     server      │ ──────▶ shared + protocol + encryption (仅密钥交换相关类型)
└─────────────────┘
```

### 依赖关系说明

| 包名           | 依赖                         | 说明                                           |
| -------------- | ---------------------------- | ---------------------------------------------- |
| `shared`       | 无                           | 零依赖，纯类型和工具函数                       |
| `protocol`     | shared, zod                  | 通信协议定义，包含加密消息类型                 |
| `encryption`   | shared, protocol             | 端到端加密实现，需要 protocol 中的加密消息类型 |
| `client-core`  | shared, protocol, encryption | 客户端核心，网络和存储                         |
| `client-state` | shared, client-core          | 状态管理层                                     |
| `ui-web`       | shared, client-state         | Web UI 组件，需要 shared 中的类型定义          |
| `ui-native`    | shared, client-state         | Native UI 组件，需要 shared 中的类型定义       |
| `server`       | shared, protocol             | 服务端，处理密钥交换但不解密消息               |

### 依赖版本管理

使用 `pnpm-workspace.yaml` 配合 `pnpm catalog` 统一管理依赖版本：

```yaml
# pnpm-workspace.yaml
packages:
  - "apps/*"
  - "packages/*"

catalog:
  # 核心框架
  react: latest
  react-dom: latest
  react-native: latest
  typescript: latest

  # 状态管理
  zustand: latest
  "@tanstack/react-query": latest

  # 数据验证
  zod: latest

  # 网络通信
  socket.io: latest
  socket.io-client: latest
  axios: latest

  # 后端框架
  "@nestjs/core": latest
  "@nestjs/common": latest
  "@nestjs/platform-socket.io": latest
  "@nestjs/websockets": latest
  prisma: latest
  "@prisma/client": latest

  # UI 组件库
  "@radix-ui/react-*": latest
  nativewind: latest # Gluestack UI v3 依赖
  tailwindcss: latest

  # Electron
  electron: latest
  electron-vite: latest
  electron-builder: latest

  # 加密
  "@aspect-build/libsignal-client": latest

  # 测试
  vitest: latest
  "@testing-library/react": latest
  "@testing-library/react-native": latest
```

> **注意**：生产环境建议使用 `pnpm update` 后锁定版本，确保构建可重复性。

---

## 5. 开发计划

### Phase 1: 基础设施搭建 (Week 1-2)

| 序号 | 任务                                      | 优先级 | 预计工时 |
| ---- | ----------------------------------------- | ------ | -------- |
| 1.1  | 初始化 monorepo 结构，配置 pnpm workspace | P0     | 2h       |
| 1.2  | 配置 Turborepo 构建编排                   | P0     | 2h       |
| 1.3  | 创建 `packages/typescript-config`         | P0     | 1h       |
| 1.4  | 创建 `packages/eslint-config`             | P0     | 1h       |
| 1.5  | 创建 `packages/prettier-config`           | P0     | 0.5h     |
| 1.6  | 创建 `packages/shared` 基础类型定义       | P0     | 4h       |
| 1.7  | 创建 `packages/protocol` WebSocket 事件   | P0     | 4h       |
| 1.8  | 配置 CI/CD 流水线                         | P1     | 4h       |

### Phase 2: 后端核心开发 (Week 3-6)

| 序号 | 任务                                  | 优先级 | 预计工时 |
| ---- | ------------------------------------- | ------ | -------- |
| 2.1  | 初始化 Nest.js 项目结构               | P0     | 2h       |
| 2.2  | 配置 Prisma + 数据库连接              | P0     | 2h       |
| 2.3  | 设计并实现数据库 Schema               | P0     | 16h      |
| 2.4  | 实现 `auth` 模块 (JWT + Refresh)      | P0     | 8h       |
| 2.5  | 实现 `user` 模块                      | P0     | 4h       |
| 2.6  | 实现 `friend` 模块                    | P0     | 6h       |
| 2.7  | 实现 `chat` 模块                      | P0     | 6h       |
| 2.8  | 实现 `group` 模块 (成员管理/权限控制) | P0     | 16h      |
| 2.9  | 实现 `message` 模块                   | P0     | 8h       |
| 2.10 | 实现 WebSocket 网关                   | P0     | 8h       |
| 2.11 | 实现 `file` 模块 (分片上传/断点续传)  | P1     | 12h      |
| 2.12 | 实现 `notification` 模块 (FCM/APNs)   | P1     | 8h       |

### Phase 3: 加密模块开发 (Week 5-7)

> **说明**：加密模块提前开发，避免后期集成时大规模重构

| 序号 | 任务                                  | 优先级 | 预计工时 |
| ---- | ------------------------------------- | ------ | -------- |
| 3.1  | 创建 `packages/encryption` 结构       | P0     | 2h       |
| 3.2  | 实现密钥生成与管理 (libsignal-client) | P0     | 12h      |
| 3.3  | 实现 Double Ratchet 算法集成          | P0     | 24h      |
| 3.4  | 实现消息加解密流程                    | P0     | 12h      |
| 3.5  | 实现服务端密钥交换接口                | P0     | 8h       |
| 3.6  | 编写加密模块单元测试                  | P0     | 8h       |

### Phase 4: 客户端核心层开发 (Week 7-8)

| 序号 | 任务                             | 优先级 | 预计工时 |
| ---- | -------------------------------- | ------ | -------- |
| 4.1  | 创建 `packages/client-core` 结构 | P0     | 2h       |
| 4.2  | 实现 HTTP API 封装 (Axios)       | P0     | 4h       |
| 4.3  | 实现 WebSocket 客户端封装        | P0     | 6h       |
| 4.4  | 实现存储适配器接口               | P0     | 2h       |
| 4.5  | 实现 Web 存储适配器 (IndexedDB)  | P0     | 4h       |
| 4.6  | 实现 Native 存储适配器 (MMKV)    | P0     | 4h       |
| 4.7  | 集成加密模块到 client-core       | P0     | 8h       |
| 4.8  | 实现消息同步策略                 | P0     | 12h      |
| 4.9  | 实现离线消息队列                 | P1     | 8h       |

### Phase 5: 状态管理层开发 (Week 8-9)

| 序号 | 任务                              | 优先级 | 预计工时 |
| ---- | --------------------------------- | ------ | -------- |
| 5.1  | 创建 `packages/client-state` 结构 | P0     | 1h       |
| 5.2  | 实现 `auth.store`                 | P0     | 4h       |
| 5.3  | 实现 `user.store`                 | P0     | 2h       |
| 5.4  | 实现 `chat.store`                 | P0     | 4h       |
| 5.5  | 实现 `message.store`              | P0     | 6h       |
| 5.6  | 实现 `ui.store`                   | P1     | 2h       |
| 5.7  | 实现业务 Hooks                    | P0     | 8h       |
| 5.8  | 实现 TanStack Query 配置          | P0     | 4h       |

### Phase 6: UI 组件库开发 (Week 9-11)

| 序号 | 任务                                  | 优先级 | 预计工时 |
| ---- | ------------------------------------- | ------ | -------- |
| 6.1  | 设计 Design Token（两端统一设计语言） | P0     | 4h       |
| 6.2  | 配置 shadcn/ui + Tailwind             | P0     | 2h       |
| 6.3  | 实现 `ui-web` 基础组件                | P0     | 12h      |
| 6.4  | 实现 `ui-web` 聊天组件（自定义）      | P0     | 24h      |
| 6.5  | 实现 `ui-web` 布局组件                | P0     | 8h       |
| 6.6  | 配置 Gluestack UI                     | P0     | 2h       |
| 6.7  | 实现 `ui-native` 基础组件             | P0     | 12h      |
| 6.8  | 实现 `ui-native` 聊天组件（自定义）   | P0     | 24h      |
| 6.9  | 实现 `ui-native` 布局组件             | P0     | 8h       |

### Phase 7: 桌面端应用开发 (Week 11-13)

| 序号 | 任务                        | 优先级 | 预计工时 |
| ---- | --------------------------- | ------ | -------- |
| 7.1  | 初始化 Electron + Vite 项目 | P0     | 4h       |
| 7.2  | 配置 electron-vite          | P0     | 2h       |
| 7.3  | 实现主进程基础结构          | P0     | 4h       |
| 7.4  | 实现 Preload 安全桥接       | P0     | 4h       |
| 7.5  | 实现 IPC 通信               | P0     | 6h       |
| 7.6  | 实现系统托盘                | P1     | 4h       |
| 7.7  | 实现自动更新                | P1     | 6h       |
| 7.8  | 实现所有页面                | P0     | 32h      |
| 7.9  | 打包配置 (Windows/macOS)    | P0     | 8h       |

### Phase 8: 移动端应用开发 (Week 13-16)

| 序号 | 任务                       | 优先级 | 预计工时 |
| ---- | -------------------------- | ------ | -------- |
| 8.1  | 初始化 React Native 项目   | P0     | 4h       |
| 8.2  | 配置 Metro (monorepo 兼容) | P0     | 6h       |
| 8.3  | 配置 Gluestack UI 主题     | P0     | 2h       |
| 8.4  | 实现导航结构               | P0     | 4h       |
| 8.5  | 实现原生模块桥接           | P0     | 8h       |
| 8.6  | 集成推送通知 (FCM/APNs)    | P0     | 12h      |
| 8.7  | 实现生物识别认证           | P1     | 6h       |
| 8.8  | 实现后台消息同步           | P1     | 8h       |
| 8.9  | 实现所有屏幕               | P0     | 32h      |
| 8.10 | iOS 打包配置               | P0     | 6h       |
| 8.11 | Android 打包配置           | P0     | 6h       |

### Phase 9: 测试与优化 (Week 16-18)

| 序号 | 任务                    | 优先级 | 预计工时 |
| ---- | ----------------------- | ------ | -------- |
| 9.1  | 创建 `packages/testing` | P0     | 4h       |
| 9.2  | 编写单元测试            | P0     | 24h      |
| 9.3  | 编写集成测试            | P0     | 16h      |
| 9.4  | 编写 E2E 测试           | P1     | 16h      |
| 9.5  | 性能优化                | P1     | 16h      |
| 9.6  | 安全审计                | P0     | 8h       |

### Phase 10: 缓冲与发布 (Week 19-20)

> **说明**：预留缓冲时间用于 Bug 修复、集成测试和应用商店审核

| 序号 | 任务                             | 优先级 | 预计工时 |
| ---- | -------------------------------- | ------ | -------- |
| 10.1 | Bug 修复与优化                   | P0     | 16h      |
| 10.2 | 全平台集成测试                   | P0     | 16h      |
| 10.3 | 文档完善                         | P1     | 8h       |
| 10.4 | App Store / Google Play 审核提交 | P0     | 8h       |
| 10.5 | 正式发布                         | P0     | 4h       |

---

## 6. 模块详细说明

### 6.1 packages/protocol

**职责**：定义前后端共享的 WebSocket 事件和数据验证 schema

```typescript
// packages/protocol/src/events/message.events.ts
export enum MessageEvent {
  // 客户端 -> 服务端
  SEND = "message:send",
  EDIT = "message:edit",
  DELETE = "message:delete",
  READ = "message:read",
  TYPING_START = "message:typing:start",
  TYPING_STOP = "message:typing:stop",

  // 服务端 -> 客户端
  RECEIVE = "message:receive",
  DELIVERED = "message:delivered",
  UPDATED = "message:updated",
  DELETED = "message:deleted",
  READ_RECEIPT = "message:read:receipt",
  TYPING = "message:typing",
}

// packages/protocol/src/schemas/message.schema.ts
import { z } from "zod";

export const sendMessageSchema = z.object({
  conversationId: z.string().uuid(),
  content: z.string().min(1).max(10000),
  type: z.enum(["text", "image", "file", "voice", "video"]),
  replyTo: z.string().uuid().optional(),
  attachments: z
    .array(
      z.object({
        id: z.string(),
        type: z.string(),
        url: z.string().url(),
        name: z.string(),
        size: z.number(),
      })
    )
    .optional(),
});

export type SendMessagePayload = z.infer<typeof sendMessageSchema>;
```

### 6.2 packages/client-core

**职责**：提供跨平台的网络请求、WebSocket 连接、本地存储抽象

```typescript
// packages/client-core/src/storage/adapter.ts
export interface StorageAdapter {
  get<T>(key: string): Promise<T | null>;
  set<T>(key: string, value: T): Promise<void>;
  delete(key: string): Promise<void>;
  clear(): Promise<void>;

  // 批量操作
  multiGet<T>(keys: string[]): Promise<Map<string, T>>;
  multiSet<T>(entries: [string, T][]): Promise<void>;
}

// packages/client-core/src/socket/socket-client.ts
export class SocketClient {
  private socket: Socket;
  private reconnectAttempts = 0;

  connect(token: string): Promise<void>;
  disconnect(): void;

  emit<T>(event: string, payload: T): void;
  on<T>(event: string, handler: (payload: T) => void): void;
  off(event: string, handler?: Function): void;

  // 连接状态
  get isConnected(): boolean;
  onConnectionChange(handler: (connected: boolean) => void): void;
}
```

### 6.3 packages/client-state

**职责**：统一管理客户端状态，提供业务 Hooks

```typescript
// packages/client-state/src/stores/message.store.ts
import { create } from "zustand";
import { immer } from "zustand/middleware/immer";

interface MessageState {
  messages: Map<string, Message[]>; // conversationId -> messages
  pendingMessages: Message[];

  // Actions
  addMessage: (conversationId: string, message: Message) => void;
  updateMessage: (messageId: string, updates: Partial<Message>) => void;
  deleteMessage: (messageId: string) => void;
  markAsRead: (conversationId: string, messageId: string) => void;
}

export const useMessageStore = create<MessageState>()(
  immer((set) => ({
    messages: new Map(),
    pendingMessages: [],

    addMessage: (conversationId, message) =>
      set((state) => {
        const existing = state.messages.get(conversationId) || [];
        state.messages.set(conversationId, [...existing, message]);
      }),

    // ... 其他 actions
  }))
);

// packages/client-state/src/hooks/useMessages.ts
export function useMessages(conversationId: string) {
  const messages = useMessageStore(
    (state) => state.messages.get(conversationId) || []
  );
  const addMessage = useMessageStore((state) => state.addMessage);
  const socket = useSocket();

  const sendMessage = useCallback(
    async (content: string) => {
      const message = createOptimisticMessage(content);
      addMessage(conversationId, message);

      socket.emit(MessageEvent.SEND, {
        conversationId,
        content,
        type: "text",
      });
    },
    [conversationId, addMessage, socket]
  );

  return { messages, sendMessage };
}
```

### 6.4 packages/encryption

**职责**：实现端到端加密，保护用户隐私

```typescript
// packages/encryption/src/signal/session.ts
export class SignalSession {
  private identityKeyPair: KeyPair;
  private preKeys: PreKey[];
  private sessions: Map<string, SessionState>;

  // 初始化密钥
  async initialize(): Promise<void>;

  // 建立会话
  async createSession(
    recipientId: string,
    preKeyBundle: PreKeyBundle
  ): Promise<void>;

  // 加密消息
  async encrypt(
    recipientId: string,
    plaintext: string
  ): Promise<EncryptedMessage>;

  // 解密消息
  async decrypt(
    senderId: string,
    ciphertext: EncryptedMessage
  ): Promise<string>;
}
```

### 6.5 packages/ui-web (shadcn/ui)

**职责**：Web/Electron 端 UI 组件库

```typescript
// packages/ui-web/tsconfig.json 中配置路径别名
// "paths": { "@/*": ["./src/*"] }

// packages/ui-web/src/components/base/Button/Button.tsx
// 使用 shadcn/ui 的 Button 组件
import { Button as ShadcnButton } from "@/components/ui/button";
import { cn } from "@/lib/utils";

interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?:
    | "default"
    | "destructive"
    | "outline"
    | "secondary"
    | "ghost"
    | "link";
  size?: "default" | "sm" | "lg" | "icon";
}

export function Button({ className, variant, size, ...props }: ButtonProps) {
  return (
    <ShadcnButton
      className={cn(className)}
      variant={variant}
      size={size}
      {...props}
    />
  );
}

// packages/ui-web/src/components/chat/ChatBubble/ChatBubble.tsx
// 聊天气泡 - 自定义实现
import type { Message } from "@cyan/shared";

interface ChatBubbleProps {
  message: Message;
  isOwn: boolean;
  showAvatar?: boolean;
}

export function ChatBubble({ message, isOwn, showAvatar }: ChatBubbleProps) {
  return (
    <div className={cn("flex gap-2", isOwn && "flex-row-reverse")}>
      {showAvatar && <Avatar src={message.sender.avatar} />}
      <div
        className={cn(
          "max-w-[70%] rounded-2xl px-4 py-2",
          isOwn ? "bg-primary text-primary-foreground" : "bg-muted"
        )}
      >
        <p className="text-sm">{message.content}</p>
        <span className="text-xs opacity-70">
          {formatTime(message.createdAt)}
        </span>
      </div>
    </div>
  );
}
```

### 6.6 packages/ui-native (Gluestack UI)

**职责**：React Native 端 UI 组件库

```typescript
// packages/ui-native/src/components/base/Button/Button.tsx
// Gluestack UI v3 采用 copy-paste 模式，组件从本地导入
import { Button as GluestackButton, ButtonText } from "@/components/ui/button";
import type { Message } from "@cyan/shared";

interface ButtonProps {
  children: string;
  variant?: "solid" | "outline" | "link";
  size?: "xs" | "sm" | "md" | "lg" | "xl";
  onPress?: () => void;
}

export function Button({
  children,
  variant = "solid",
  size = "md",
  onPress,
}: ButtonProps) {
  return (
    <GluestackButton variant={variant} size={size} onPress={onPress}>
      <ButtonText>{children}</ButtonText>
    </GluestackButton>
  );
}

// packages/ui-native/src/components/chat/ChatBubble/ChatBubble.tsx
import { Box } from "@/components/ui/box";
import { Text } from "@/components/ui/text";
import { HStack } from "@/components/ui/hstack";
import { Avatar } from "@/components/ui/avatar";
import type { Message } from "@cyan/shared";

interface ChatBubbleProps {
  message: Message;
  isOwn: boolean;
  showAvatar?: boolean;
}

export function ChatBubble({ message, isOwn, showAvatar }: ChatBubbleProps) {
  return (
    <HStack space="sm" className={isOwn ? "justify-end" : "justify-start"}>
      {showAvatar && !isOwn && (
        <Avatar source={{ uri: message.sender.avatar }} />
      )}
      <Box
        className={`max-w-[70%] rounded-2xl px-4 py-2 ${
          isOwn ? "bg-primary-500" : "bg-background-200"
        }`}
      >
        <Text className={isOwn ? "text-white" : "text-typography-900"}>
          {message.content}
        </Text>
        <Text size="xs" className="opacity-70">
          {formatTime(message.createdAt)}
        </Text>
      </Box>
    </HStack>
  );
}
```

### 6.7 apps/server

**职责**：提供 REST API 和 WebSocket 实时通信

```typescript
// apps/server/src/gateway/chat.gateway.ts
@WebSocketGateway({
  namespace: "chat",
  cors: { origin: "*", credentials: true },
})
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  @WebSocketServer()
  server: Namespace;

  constructor(
    private messageService: MessageService,
    private userService: UserService
  ) {}

  async handleConnection(client: Socket) {
    const userId = await this.validateToken(client);
    if (!userId) {
      client.disconnect();
      return;
    }

    // 加入用户房间，支持多设备
    client.join(`user:${userId}`);
    await this.userService.setOnline(userId);
  }

  @SubscribeMessage(MessageEvent.SEND)
  @UseGuards(WsAuthGuard)
  async handleSendMessage(
    @MessageBody() payload: SendMessagePayload,
    @ConnectedSocket() client: Socket
  ) {
    // 验证 payload
    const validated = sendMessageSchema.parse(payload);

    // 保存消息
    const message = await this.messageService.create(validated);

    // 广播给会话成员
    const recipientIds = await this.getConversationMembers(
      validated.conversationId
    );
    for (const recipientId of recipientIds) {
      this.server.to(`user:${recipientId}`).emit(MessageEvent.RECEIVE, message);
    }

    return { success: true, messageId: message.id };
  }
}
```

---

## 7. 消息同步与离线策略

### 7.1 消息同步架构

```
┌─────────────────────────────────────────────────────────────────┐
│                        消息同步策略                               │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐         │
│  │  实时推送    │    │  增量同步    │    │  全量同步    │         │
│  │  (WebSocket) │    │  (Cursor)   │    │  (首次登录)  │         │
│  └──────┬──────┘    └──────┬──────┘    └──────┬──────┘         │
│         │                  │                  │                │
│         └──────────────────┴──────────────────┘                │
│                            │                                    │
│                            ▼                                    │
│                   ┌─────────────────┐                          │
│                   │   本地存储       │                          │
│                   │  (IndexedDB/    │                          │
│                   │   MMKV)         │                          │
│                   └─────────────────┘                          │
└─────────────────────────────────────────────────────────────────┘
```

### 7.2 同步策略详解

#### 增量同步

```typescript
// packages/client-core/src/sync/message-sync.ts
interface SyncCursor {
  conversationId: string;
  lastMessageId: string;
  lastSyncTime: number;
}

export class MessageSyncManager {
  // 增量同步：基于游标获取新消息
  async syncIncremental(cursor: SyncCursor): Promise<Message[]> {
    const response = await this.api.get("/messages/sync", {
      params: {
        conversationId: cursor.conversationId,
        after: cursor.lastMessageId,
        limit: 100,
      },
    });

    // 更新本地游标
    await this.storage.set(`cursor:${cursor.conversationId}`, {
      lastMessageId: response.messages.at(-1)?.id,
      lastSyncTime: Date.now(),
    });

    return response.messages;
  }

  // 分页加载历史消息
  async loadHistory(
    conversationId: string,
    before: string
  ): Promise<Message[]> {
    return this.api.get("/messages/history", {
      params: { conversationId, before, limit: 50 },
    });
  }
}
```

#### 离线消息队列

```typescript
// packages/client-core/src/sync/offline-queue.ts
interface PendingMessage {
  id: string;
  payload: SendMessagePayload;
  createdAt: number;
  retryCount: number;
}

export class OfflineQueue {
  private queue: PendingMessage[] = [];

  // 添加到离线队列
  async enqueue(payload: SendMessagePayload): Promise<string> {
    const pending: PendingMessage = {
      id: generateUUID(),
      payload,
      createdAt: Date.now(),
      retryCount: 0,
    };

    this.queue.push(pending);
    await this.storage.set("offline_queue", this.queue);

    return pending.id;
  }

  // 网络恢复时批量发送
  async flush(): Promise<void> {
    const pending = [...this.queue];

    for (const message of pending) {
      try {
        await this.socket.emit(MessageEvent.SEND, message.payload);
        this.queue = this.queue.filter((m) => m.id !== message.id);
      } catch (error) {
        message.retryCount++;
        if (message.retryCount >= 3) {
          // 标记为发送失败
          this.onMessageFailed(message);
        }
      }
    }

    await this.storage.set("offline_queue", this.queue);
  }
}
```

### 7.3 多设备消息同步

```typescript
// 多设备同步策略
interface DeviceSyncState {
  deviceId: string;
  lastSeenMessageId: Map<string, string>; // conversationId -> messageId
}

// 服务端处理多设备
@SubscribeMessage(MessageEvent.SYNC_REQUEST)
async handleSyncRequest(
  @MessageBody() payload: { deviceId: string; cursors: SyncCursor[] }
) {
  // 向用户的所有设备广播（除了发送方）
  this.server.to(`user:${userId}`).except(payload.deviceId).emit(
    MessageEvent.SYNC_RESPONSE,
    { messages, cursors: updatedCursors }
  );
}
```

---

## 8. API 版本管理

### 8.1 版本策略

| 类型       | 策略         | 示例                                 |
| ---------- | ------------ | ------------------------------------ |
| REST API   | URL 版本前缀 | `/api/v1/users`, `/api/v2/users`     |
| WebSocket  | 事件命名空间 | `v1:message:send`, `v2:message:send` |
| 客户端版本 | 请求头       | `X-Client-Version: 1.2.0`            |

### 8.2 REST API 版本控制

```typescript
// apps/server/src/main.ts
import { VersioningType } from "@nestjs/common";

async function bootstrap() {
  const app = await NestFactory.create(AppModule);

  // 启用 URI 版本控制
  app.enableVersioning({
    type: VersioningType.URI,
    defaultVersion: "1",
  });

  await app.listen(3000);
}

// apps/server/src/modules/user/user.controller.ts
@Controller("users")
export class UserController {
  @Get()
  @Version("1")
  findAllV1() {
    // v1 实现
  }

  @Get()
  @Version("2")
  findAllV2() {
    // v2 实现，包含新字段
  }
}
```

### 8.3 WebSocket 事件版本控制

```typescript
// packages/protocol/src/events/message.events.ts
export const MessageEvent = {
  // V1 事件（保持兼容）
  SEND: 'v1:message:send',
  RECEIVE: 'v1:message:receive',

  // V2 事件（新功能）
  SEND_V2: 'v2:message:send',      // 支持富文本
  RECEIVE_V2: 'v2:message:receive',
} as const;

// 服务端同时支持多版本
@SubscribeMessage(MessageEvent.SEND)
handleSendV1(payload: SendMessagePayloadV1) { /* ... */ }

@SubscribeMessage(MessageEvent.SEND_V2)
handleSendV2(payload: SendMessagePayloadV2) { /* ... */ }
```

### 8.4 客户端版本协商

```typescript
// packages/client-core/src/api/client.ts
const apiClient = axios.create({
  baseURL: process.env.API_URL,
  headers: {
    "X-Client-Version": APP_VERSION,
    "X-Client-Platform": PLATFORM, // 'ios' | 'android' | 'desktop'
  },
});

// 服务端根据版本返回兼容响应
@Injectable()
export class VersionInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler) {
    const request = context.switchToHttp().getRequest();
    const clientVersion = request.headers["x-client-version"];

    return next
      .handle()
      .pipe(map((data) => this.transformForVersion(data, clientVersion)));
  }
}
```

---

## 9. 监控与可观测性

### 9.1 监控架构

```
┌─────────────────────────────────────────────────────────────────┐
│                         监控平台                                 │
├─────────────────┬─────────────────┬─────────────────────────────┤
│    Prometheus   │     Grafana     │      Sentry/Datadog         │
│    (Metrics)    │   (Dashboard)   │    (Error Tracking)         │
└────────┬────────┴────────┬────────┴────────┬────────────────────┘
         │                 │                 │
         └─────────────────┴─────────────────┘
                           │
         ┌─────────────────┴─────────────────┐
         │                                   │
         ▼                                   ▼
┌─────────────────┐                 ┌─────────────────┐
│   Server Apps   │                 │  Client Apps    │
│  - API Metrics  │                 │  - Error Report │
│  - WS Metrics   │                 │  - Performance  │
│  - DB Metrics   │                 │  - User Events  │
└─────────────────┘                 └─────────────────┘
```

### 9.2 服务端监控

```typescript
// apps/server/src/common/interceptors/metrics.interceptor.ts
import { Counter, Histogram } from "prom-client";

const httpRequestDuration = new Histogram({
  name: "http_request_duration_seconds",
  help: "Duration of HTTP requests in seconds",
  labelNames: ["method", "route", "status"],
});

const wsConnectionsGauge = new Gauge({
  name: "ws_connections_total",
  help: "Total WebSocket connections",
});

const messagesSentCounter = new Counter({
  name: "messages_sent_total",
  help: "Total messages sent",
  labelNames: ["type"],
});

@Injectable()
export class MetricsInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler) {
    const start = Date.now();
    const request = context.switchToHttp().getRequest();

    return next.handle().pipe(
      tap(() => {
        const duration = (Date.now() - start) / 1000;
        httpRequestDuration
          .labels(request.method, request.route.path, "200")
          .observe(duration);
      })
    );
  }
}
```

### 9.3 客户端错误上报

```typescript
// packages/client-core/src/monitoring/error-reporter.ts
import * as Sentry from "@sentry/react-native"; // 或 @sentry/electron

export class ErrorReporter {
  static init(config: { dsn: string; environment: string }) {
    Sentry.init({
      dsn: config.dsn,
      environment: config.environment,
      tracesSampleRate: 0.1,
      beforeSend(event) {
        // 过滤敏感信息
        return sanitizeEvent(event);
      },
    });
  }

  static captureException(error: Error, context?: Record<string, any>) {
    Sentry.withScope((scope) => {
      if (context) {
        scope.setExtras(context);
      }
      Sentry.captureException(error);
    });
  }

  static setUser(user: { id: string; username: string }) {
    Sentry.setUser({ id: user.id, username: user.username });
  }
}
```

### 9.4 性能监控

```typescript
// packages/client-core/src/monitoring/performance.ts
export class PerformanceMonitor {
  // 消息发送延迟
  static measureMessageLatency(messageId: string) {
    const start = performance.now();

    return {
      end: () => {
        const latency = performance.now() - start;
        this.report("message_latency", latency, { messageId });
      },
    };
  }

  // 启动时间
  static measureAppStartup() {
    const metrics = {
      cold_start: performance.now(),
      first_render: 0,
      interactive: 0,
    };

    return {
      markFirstRender: () => {
        metrics.first_render = performance.now();
      },
      markInteractive: () => {
        metrics.interactive = performance.now();
        this.report("app_startup", metrics);
      },
    };
  }

  private static report(
    name: string,
    value: any,
    tags?: Record<string, string>
  ) {
    // 发送到分析服务
  }
}
```

---

## 10. 开发规范

### 10.1 代码规范

- **命名约定**

  - 文件：`kebab-case.ts` (如 `message-sync.ts`)
  - 组件：`PascalCase.tsx` (如 `ChatBubble.tsx`)
  - 类型：`PascalCase` (如 `MessagePayload`)
  - 常量：`SCREAMING_SNAKE_CASE` (如 `MAX_MESSAGE_LENGTH`)

- **目录结构**
  ```
  ComponentName/
  ├── index.ts              # 导出
  ├── ComponentName.tsx     # 组件实现
  ├── ComponentName.test.tsx
  ├── ComponentName.styles.ts  # 或 .css
  └── types.ts              # 组件类型
  ```

### 10.2 Git 规范

- **分支命名**

  - `feature/xxx` - 新功能
  - `fix/xxx` - Bug 修复
  - `refactor/xxx` - 重构
  - `docs/xxx` - 文档

- **Commit 格式**

  ```
  <type>(<scope>): <subject>

  <body>

  <footer>
  ```

  类型: `feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`

### 10.3 测试规范

- **覆盖率要求**

  - `packages/*`: 80%+
  - `apps/server`: 70%+
  - `apps/desktop`, `apps/mobile`: 60%+

- **测试文件命名**
  - 单元测试: `*.test.ts`
  - 集成测试: `*.integration.test.ts`
  - E2E 测试: `*.e2e.test.ts`

---

## 11. 部署架构

### 11.1 服务端部署

```
                    ┌─────────────────┐
                    │   CDN (静态资源)  │
                    └────────┬────────┘
                             │
                    ┌────────▼────────┐
                    │  Load Balancer  │
                    └────────┬────────┘
                             │
         ┌───────────────────┼───────────────────┐
         │                   │                   │
         ▼                   ▼                   ▼
┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐
│  API Server 1   │ │  API Server 2   │ │  API Server N   │
│   (Nest.js)     │ │   (Nest.js)     │ │   (Nest.js)     │
└────────┬────────┘ └────────┬────────┘ └────────┬────────┘
         │                   │                   │
         └───────────────────┼───────────────────┘
                             │
              ┌──────────────┼──────────────┐
              │              │              │
              ▼              ▼              ▼
      ┌─────────────┐ ┌─────────────┐ ┌─────────────┐
      │ PostgreSQL  │ │    Redis    │ │     S3      │
      │  (Primary)  │ │  (Cluster)  │ │  (Storage)  │
      └─────────────┘ └─────────────┘ └─────────────┘
              │
              ▼
      ┌─────────────┐
      │ PostgreSQL  │
      │  (Replica)  │
      └─────────────┘
```

### 11.2 客户端发布

| 平台    | 发布渠道               | 自动更新             |
| ------- | ---------------------- | -------------------- |
| Windows | GitHub Releases / 官网 | electron-updater     |
| macOS   | GitHub Releases / 官网 | electron-updater     |
| iOS     | App Store              | App Store            |
| Android | Google Play / APK      | Google Play / In-app |

### 11.3 环境配置

```bash
# .env.example

# Server
NODE_ENV=development
PORT=3000
DATABASE_URL=postgresql://user:pass@localhost:5432/cyan
REDIS_URL=redis://localhost:6379

# JWT
JWT_SECRET=your-secret-key
JWT_ACCESS_EXPIRES=15m
JWT_REFRESH_EXPIRES=7d

# Push Notifications
FCM_SERVER_KEY=your-fcm-key
APNS_KEY_ID=your-apns-key-id
APNS_TEAM_ID=your-team-id

# Storage
S3_BUCKET=cyan-uploads
S3_REGION=ap-northeast-1
S3_ACCESS_KEY=your-access-key
S3_SECRET_KEY=your-secret-key

# Monitoring
SENTRY_DSN=your-sentry-dsn
PROMETHEUS_PORT=9090
```

---

## 附录

### A. 技术选型理由

| 技术                | 选型理由                                                       |
| ------------------- | -------------------------------------------------------------- |
| **pnpm**            | 更快的安装速度，严格的依赖管理，天然支持 monorepo              |
| **Turborepo**       | 增量构建，远程缓存，任务编排                                   |
| **Zustand**         | 轻量、简单、TypeScript 友好，适合中型应用                      |
| **TanStack Query**  | 强大的服务端状态管理，自动缓存和重试                           |
| **Zod**             | 运行时类型验证，与 TypeScript 无缝集成                         |
| **Prisma**          | 类型安全的 ORM，优秀的开发体验                                 |
| **Socket.io**       | 成熟的 WebSocket 库，自动重连，房间支持                        |
| **shadcn/ui**       | 基于 Radix + Tailwind，可复制代码，完全可控，无依赖锁定        |
| **Gluestack UI v3** | 类似 shadcn/ui 的 copy-paste 模式，基于 NativeWind，跨平台一致 |
| **libsignal**       | Signal 官方加密库，经过验证的端到端加密实现                    |

### B. 参考资源

- [Turborepo 文档](https://turbo.build/repo/docs)
- [pnpm Workspace](https://pnpm.io/workspaces)
- [Electron Vite](https://electron-vite.org/)
- [React Native Monorepo](https://reactnative.dev/docs/environment-setup)
- [Nest.js 文档](https://docs.nestjs.com/)
- [Signal Protocol](https://signal.org/docs/)
- [shadcn/ui 文档](https://ui.shadcn.com/)
- [Gluestack UI 文档](https://gluestack.io/)
- [Prometheus 监控](https://prometheus.io/docs/)
- [Sentry 错误追踪](https://docs.sentry.io/)

---

_文档版本: 2.0.0_
_最后更新: 2025-12-03_
